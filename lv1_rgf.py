...
@app.init()
def init():
    rgf.utils.Config.init_paths()
    rgf.utils.Config.TEMP_PATH = str(models_dir / "rgf")


def create_model():
    # https://github.com/RGF-team/rgf/tree/master/python-package
    return tk.pipeline.SKLearnModel(
        estimator=rgf.sklearn.RGFClassifier(),
        nfold=nfold,
...
def train():
    ...
    rgf_temp_path = pathlib.Path(rgf.utils.Config.TEMP_PATH)
    if rgf_temp_path.is_dir():
        shutil.rmtree(rgf_temp_path)
    rgf_temp_path.mkdir(parents=True, exist_ok=True)
